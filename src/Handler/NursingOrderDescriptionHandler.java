...
public class NursingOrderDescriptionHandler implements java.io.Serializable {
    ...
    public List<OfflineOrderData> selectInvoiceMaster(String admissionNo,
        ...
        String[] selectColumns = {"-", "PATIENT_ID", "FULL_NAME", "COMPLETE_ORDER_NO",
            ...
            "MANUAL_INVOICE_NO", "USERNAME", "TERMINAL_ID", "INVOICE_BY", "USER_ID",
        ...
        String selectInvoiceMaster
                ...
                + "WHERE IM.ADMISSION_NO = '" + admissionNo + "'";
        if (!status.isEmpty()) {
            selectInvoiceMaster += " AND IM.STATUS_ID = '" + status + "' \n";
        }

        if (!userName.isEmpty()) {
            selectInvoiceMaster += " AND USR.USER_NAME = '" + userName + "' \n";
        }

        if (!fromDate.isEmpty() || !toDate.isEmpty()) {
            selectInvoiceMaster += " AND IM.TRN_DATE BETWEEN '" + fromDate + "' AND '" + toDate + "' \n";
        }

        selectInvoiceMaster += " AND PAT.PATIENT_ID   = IM.PATIENT_ID        \n"
...
//            set.setUserName(map.get("USERNAME").toString());
