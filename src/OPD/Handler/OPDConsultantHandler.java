...
public class OPDConsultantHandler {
          ...
          String query ="SELECT COUNT(OPC.ID) TOTAL,                         \n"
                 ...
                 +Database.DCMS.invoiceDetail+"IVD,                          \n"
                  ...
                  if(fromDate.length() != 0 && toDate.length() != 0){
                      query += "AND TRUNC(OPC.CRTD_DATE) BETWEEN '"+fromDate+"' AND '"+toDate+"' \n";
                  }
     ...
     public List<BOOPDConsultant> selectPatientHistory(String fromDate, String toDate,String userId){
         ...
         String columns[] = {"-","PATIENT_ID","PATIENT_NAME", "INVOICE_NO", "ORDER_STATUS_ID","STATUS",
         ...
         String query ="SELECT SUBSTR(OPC.PATIENT_ID,'4')PATIENT_ID,         \n"
                 ...
                 +"AND TRUNC(OPC.CRTD_DATE) BETWEEN '"+fromDate+"' AND '"+toDate+"' \n"
                 +"AND OPC.PATIENT_ID = PAT.PATIENT_ID                       \n"
                 +"AND OPC.CONSULTATION_ROOM_ID = ROM.ID                     \n"
                 +"AND OPC.ORDER_STATUS_ID = STA.ID                          \n"
                 +"AND OPC.ORDER_STATUS_ID != 14                             \n"
     ...
     public List<BOOPDConsultant> selectTotalPerform(String fromDate, String toDate,String userId){
          ...
          String columns[] = {"-","TOTAL_PERFORM","ORDER_STATUS_ID", "STATUS"};
           String query =" SELECT COUNT(OPC.CONSULTANT_ID) TOTAL_PERFORM,    \n"
                 +" OPC.ORDER_STATUS_ID                    ORDER_STATUS_ID,  \n"
                 +" STA.DESCRIPTION                        STATUS            \n"
                 +" FROM " +Database.DCMS.opdPerformedConsultancy+" OPC,     \n"
                 +Database.DCMS.definitionTypeDetail+"STA                    \n" 
                 ...
                 +"AND TRUNC(OPC.CRTD_DATE) BETWEEN '"+fromDate+"' AND '"+toDate+"'  \n"
                 +"AND OPC.ORDER_STATUS_ID = STA.ID                          \n"
                 +"AND OPC.ORDER_STATUS_ID = 12                              \n"
                 +"GROUP BY  OPC.ORDER_STATUS_ID,STA.DESCRIPTION             \n"
                 +"ORDER BY OPC.ORDER_STATUS_ID   ASC                        \n";
          ...
          for (int i =0; i < listmap.size(); i++){
              ...
              objData.setTotalPerform(map.get("TOTAL_PERFORM").toString());
              objData.setOrderStatusId(map.get("ORDER_STATUS_ID").toString());
              objData.setStatus(map.get("STATUS").toString());
              lisVerRep.add(objData);
         }
