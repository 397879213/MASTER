...
public class RefundDetailTableModel extends AbstractTableModel {

     private String[] columnNames = 
                {"Sr","Refund No","Refund By","Refund Date","MR.No","Name","Description", 
                    "Price","Discount","Refd.Amount","Fin.Support","Inv.Adj.","Ldgr.Adj", "Cash.Refd"};
   ...
   private Object[][] data;
    ...
    public RefundDetailTableModel(List<DailyCashSummary> orders) {
        ...
        for (int i = 0; i<orders.size(); i++) {
           DailyCashSummary order = orders.get(i);
            ...
            data[row][7] = order.getPrice();
            data[row][8] = order.getNetDiscount();
            data[row][9] = order.getRefundAmount();
            data[row][10] = order.getSupport();
            data[row][11] = order.getInvoiceAdjusted();
            data[row][12] = order.getLedgerAdjusted();
            data[row][13] = order.getCashRefund();
             row++;
