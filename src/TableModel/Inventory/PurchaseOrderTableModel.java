...
public class PurchaseOrderTableModel extends AbstractTableModel {
    ...
    public PurchaseOrderTableModel(List<PurchaseOrderDetailBO> li) {
        data = new Object[li.size()][columnNames.length];
        for (int i = 0; i < li.size(); i++) {
            PurchaseOrderDetailBO dS = li.get(i);
            data[i][0] = (i+1);
            data[i][1] = dS.getPurchaseOrderNo();
            data[i][2] = dS.getPartyName();
            data[i][3] = dS.getStatus();
            data[i][4] = dS.getApprovedDate();
            data[i][5] = dS.getApproveBy();
