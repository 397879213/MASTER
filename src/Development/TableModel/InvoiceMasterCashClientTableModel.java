...
public class InvoiceMasterCashClientTableModel extends AbstractTableModel {

     private String[] columnNames = 
                {"Sr","Invoice No","Invoice By","Invoice Date","MR.No","Name", "Client Name","Total Amount","Discount", 
                    "Payable", "Fin.Support","Adv.Recieved","Recv.Amount"};
   ...
   private Object[][] data;
    ...
    public InvoiceMasterCashClientTableModel(List<DailyCashSummary> orders) {
        ...
        for (int i = 0; i<orders.size(); i++) {
           DailyCashSummary order = orders.get(i);
            ...
            data[row][6] = order.getClientName();
            data[row][7] = order.getTotalAmount();
            data[row][8] = order.getNetDiscount();
            data[row][9] = order.getPayableAmount();
            data[row][10] = order.getSupport();
            data[row][11] = order.getAdvanceAdjusted();
